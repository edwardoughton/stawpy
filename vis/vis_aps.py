...
def pairwise(data, folder, filename):
    ...
    path = os.path.join(folder, "scatter_{}.png".format(filename))
    pairplot.savefig(path)
    plt.clf()

    plt.figure()
    pairplot = sns.pairplot(data, kind = "reg", palette = 'husl')
    path = os.path.join(folder, "reg_{}.png".format(filename))
    pairplot.savefig(path)
    plt.clf()
...
def histograms(data, folder, side_length):
    ...
    # path = os.path.join(folder, "histograms_building_count_{}.png".format(side_length))
    # plot.savefig(path)
    # plt.clf()
    ...
    # plot.set_xticklabels(rotation=45)
    # path = os.path.join(folder, "histograms_floor_area_{}.png".format(side_length))
    # plot.savefig(path)
    # plt.clf()

    # plt.figure()
    # plot = sns.catplot(x="floor_area", y="waps_collected", col="geotype", kind="box", data=data)
    # # plot = sns.swarmplot(x="floor_area", y="waps_collected", col="geotype", data=data, color=".25")
